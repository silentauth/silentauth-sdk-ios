...
final class TraceCollector {
    ...
    let queue = DispatchQueue(label: "silentauth.tracecollector.queue")
    ...
    func addDebug(type: OSLogType = .debug, tag: String? = "SilentAuth", log: String) {
        queue.async {
...
public class DebugInfo {
    ...
    public func userAgent(sdkVersion: String) -> String {
        return "silentauth-sdk-ios/\(sdkVersion) \(deviceString())"
    }
